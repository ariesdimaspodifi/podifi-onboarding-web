...
type ItemCardType = {
  ...
  /** Action props */
  onItemCardContainerClick?: (menuItemCode:string) => void;
};
...
const ItemCard: FunctionComponent<ItemCardType> = ({
  ...
  return (
    <div
      ...
      onClick={() => onItemCardContainerClick && onItemCardContainerClick(menuItemCode || "")}
    >
      <div
      ...
      <div className="flex flex-row items-start justify-start text-5xl">
        <b className="relative">{itemPrice ? `$${itemPrice}` : " - "}</b>
      </div>
    </div>
  );
};
